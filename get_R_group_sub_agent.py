...
def _run_image_tool_agent_with_results(
    ...
    *,
    ...
    unknown_tool_policy="raise",
    ignore_tool_arguments=False,
    fallback_on_no_tool_call=False,
    temperature=None,
):
    ...
    if fallback_on_no_tool_call and not cache:
        return {}, result_messages, cache
...
def process_reaction_image_with_product_variant_R_group_OS(
    ...
    *,
    ...
    gpt_output = _compensate_missing_molecules(
        gpt_output,
        results,
        'get_multi_molecular_text_to_correct',
    )
    image = Image.open(image_path).convert('RGB')
    image_np = np.array(image)
...
def process_reaction_image_with_table_R_group(image_path: str) -> dict:
    ...
    def replace_symbols_and_generate_smiles(input1, input2):
        ...
        if not isinstance(input2, dict):
            raise ValueError(
                f"Expected input2 to be a dict, but got {type(input2)}: {input2}"
            )
        if 'reactions' not in input2:
            raise KeyError(
                "'reactions' key not found in model response; refusing to "
                "treat malformed output as an empty extraction"
            )
