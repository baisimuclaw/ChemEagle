...
def _chemeagle_cloud_impl(
    ...
    *,
    ...
    configured_model = (
        getattr(getattr(backend, "config", None), "model", None)
        or llm_model
    )
    if local_mode and not configured_model:
        configured_model = "Qwen/Qwen3-VL-32B-Instruct"
    if local_mode:
        ...
        from get_observer import (
            action_observer_agent_OS as action_observer_agent,
            plan_observer_agent_OS as plan_observer_agent,
        )
        from get_reaction_agent import get_reaction_con_OS as get_reaction_con
        from get_text_agent import text_extraction_agent_OS as text_extraction_agent
    else:
        from get_R_group_sub_agent import (
    ...
    planner_response = backend.generate(LLMRequest(
        ...
        temperature=0 if local_mode else None,
    ))
    ...
    if use_plan_observer:
        observer_kwargs = {"model_name": configured_model} if local_mode else {}
        observer_output = plan_observer_agent(
            image_path,
            agent_list,
            **observer_kwargs,
        )
    ...
    def _agent_kwargs(agent_name: str, *, graphical_input=None) -> dict:
        kwargs = {"image_path": image_path}
        if local_mode and agent_name in LOCAL_AGENTS_ACCEPT_MODEL:
            kwargs["model_name"] = configured_model
        if agent_name == "text_extraction_agent" and graphical_input is not None:
            kwargs["graphical_input"] = graphical_input
        return kwargs
    ...
    def _observe_and_retry(observed_name, observed_result, rerun):
        ...
        check = action_observer_agent(
            image_path,
            [{"name": observed_name, "result": observed_result}],
            **observer_kwargs,
        )
        if check.get("redo"):
    ...
    for idx, agent_name in enumerate(ordered_agents):
        ...
        try:
            agent_result = AGENT_MAP[agent_name](**_agent_kwargs(agent_name))
            if use_action_observer:
                agent_result = _observe_and_retry(
                    agent_name, agent_result,
                    lambda: AGENT_MAP[agent_name](**_agent_kwargs(agent_name)))
