...
class VisionConfig:
        ...
        **overrides: object,
        ...
        defaults = dict(
            ...
            chemrxn_device=values.get(
                "CHEMEAGLE_VISION_CHEMRXN_DEVICE", "auto"
            ).strip().lower(),
            ...
            max_attempts=min(
                3,
                max(1, int(values.get("CHEMEAGLE_VISION_MAX_ATTEMPTS", "3"))),
            ),
    ...
    def validate_remote(self) -> None:
        ...
        if not 1 <= self.max_attempts <= 3:
            raise ValueError("Vision max attempts must be between 1 and 3")
