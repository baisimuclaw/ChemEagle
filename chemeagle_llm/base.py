...
def remap_fallback_tool_result(
    result: Any,
    fallback_name: str,
    requested_name: str,
) -> Any:
    """Preserve the upstream result key when one safe tool backs a drifted name."""
    if isinstance(result, LLMToolOutput):
        return LLMToolOutput(
            value=remap_fallback_tool_result(
                result.value,
                fallback_name,
                requested_name,
            ),
            supplemental_content=list(result.supplemental_content),
        )
    if isinstance(result, dict) and fallback_name in result:
        remapped = dict(result)
        remapped[requested_name] = remapped.pop(fallback_name)
        return remapped
    return result
...
class BaseLLMBackend(ABC):
    ...
    def run_tool_loop(
        ...
        if not first.tool_calls:
            ...
            if request.followup_on_no_tool_call:
        ...
        for call in first.tool_calls:
            ...
            fallback_name = None
            if handler is None:
                if request.unknown_tool_policy == "fallback" and len(executor) == 1:
                    fallback_name, handler = next(iter(executor.items()))
                elif request.unknown_tool_policy == "skip":
                    tool_messages.append(
                        {
                            ...
                            "content": json.dumps(
                                {"error": f"Unknown tool {call.name!r} was skipped"},
                                ensure_ascii=False,
                            ),
                        }
                    )
            ...
            arguments = {} if request.ignore_tool_arguments else call.parsed_arguments()
            try:
                ...
                if fallback_name is not None:
                    result = remap_fallback_tool_result(
                        result,
                        fallback_name,
                        call.name,
                    )
        ...
        if not supplemental_content and request.tool_followup_content:
